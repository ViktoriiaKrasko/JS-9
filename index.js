...
function findElementGreaterThan(arr, num) {
  if (!Array.isArray(arr)) {
    ...
    const foundNumber = arr.find((el) => el > num);
    return foundNumber;
...
// console.log(findElementGreaterThan([3, 2, 12, 45, 54, 13], 4)); // Виведе 4
...
function findIndexByProperty(arr, name) {
  if (!Array.isArray(arr)) {
    ...
    const foundIndex = arr.findIndex((el) => el.name === name);
    return foundIndex;
...
// console.log(findLastEvenNumber([1, 3, 33, 25, 15])); // Виведе 4
// console.log(findLastEvenNumber("Three"));
...
function getPositiveArrayValues(arr) {
  if (!Array.isArray(arr)) {
    ...
    const positiveArrayValues = []; // Конвертуємо ітератор в масив.
    for (const el of iter) {
      positiveArrayValues.push(el);
    }
    // За допомогою методу filter() отримуємо масив лише з додатніми значеннями.
    return positiveArrayValues.filter((el) => el > 0); // Повертаємо масив додатніх значень.
  }
}
...
function sumAndPrint(arr) {
  if (!Array.isArray(arr)) {
    ...
    const sum = arr.reduce((el, value) => el + value, 0);
...
function getLastElementsSumAndPrint(arr, n) {
  if (!Array.isArray(arr)) {
    ...
    const sum = newArr.reduceRight((el, value) => el + value, 0);
...
function expandArrayByN(arr, n) {
  if (!Array.isArray(arr)) {
    ...
    return arr.flatMap((el) => Array(n).fill(el));
...
function findLongestWord(arr) {
  if (!Array.isArray(arr)) {
    ...
    return arr.reduce((longestWord, current) => {
...
function findDuplicateElements(arr) {
  if (!Array.isArray(arr)) {
    ...
    return arr.filter(
      (el, index) => arr.indexOf(el) !== index && arr.lastIndexOf(el) === index
    );
...
function calculateTotalPrice(arr) {
  if (!Array.isArray(arr)) {
    ...
    const totalPrice = arr.reduce((acc, el) => acc + el.price, 0);
    return totalPrice;
